...
describe.skipIf(!HEAVY_TESTS)('Heavy Integration Tests - Real Installations', () => {
  let tempDir: string;
...
describe.skipIf(!HEAVY_TESTS)('Heavy Integration Tests - Strict Mode', () => {
  ...
  it('should block --force-scripts in strict mode', async () => {
    ...
    expect(result.exitCode).toBe(1);
    expect(result.stderr).toContain('--force-scripts is blocked in strict mode');
  });
...
describe.skipIf(!HEAVY_TESTS)('Heavy Integration Tests - Script Flag Migration', () => {
  let tempDir: string;
...
describe.skipIf(!HEAVY_TESTS)('Heavy Integration Tests - Explore Security', () => {
  it('should block explore without --allow-explore', async () => {
    const result = await execa(
      'node',
      [cliPath, 'explore', 'lodash'],
      {
        reject: false,
        timeout: 30000,
      }
...
describe.skipIf(!HEAVY_TESTS)('Heavy Integration Tests - Compound Commands', () => {
  let tempDir: string;
